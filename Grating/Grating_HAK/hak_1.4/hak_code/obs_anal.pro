PRO obs_anal, filename, $
	...
	STREAK_MARX=streak_marx, $
	REANAL=reanal, SKIPSEL=skipsel, $
	EXPORT=export
;
; - - - - - - - - - Documentation - - - - - - - - - - - - - - -
;+
...
;                 array has values for the 10 chips in order:
;                 [i0,i1,...s0,s1,...,s5]
...
;                 transfer streaking.
;  /SKIPSEL  - does not do event selection steps (grade, status, etc.)
;              Useful when the input file is pre-selected (e.g., evt2)
;              or when /REANAL is used (and events have already been 
;              selected), below.
;  /REANAL   - runs obs_anal but takes the input event structure
;              from the (previously EXPORTed) oa_common oa_evnts.
;              This is useful for reprocessing with different
;              zero-order location, CD width, etc and then /SKIPSEL
;              can be used.
...
; 2000/06/30         dd acis[hrc]_pix_ang_size changed to agree with new
;                       values (0.492 vs 0.4919 for ACIS).
; 2000/07/24         dd Add period(s) to output printout.
; 2000/08/16         dd Change nominal flight Rowland spacing to 8637.00
;                       as agreed for reprocessing (will be changed again
;                       once pixel sizes and focal length, etc. are cal'ed)
; 2000/08/17         dd Added REANAL and SKIPSEL keywords...
; 2000/08/18         dd If ZBUFFER and OVERRIDE_ZERO are both selected,
;                       then popup zero-order find window to screen.
;      ----------------------------------------------
...
; 6/30/00 dd: pixel size was changed to match new pixel size
; used in CXC "2.0alpha" s/w
; arc sec, DET and Sky Coords
;;acis_pix_ang_size = 0.491900 ; mm  older value (9/99 - 4?/00)
;
acis_pix_ang_size = 0.492000 ; arc sec, DET and Sky Coords
hrc_pix_ang_size =  0.131800 ; arc sec, DET and Sky Coords
...
;
p_meg = 4001.41 ; May be increased by 150 ppm in future.
p_heg = 2000.81
p_leg = 9912.16 ;  /3.0 ; *** Look at other Order events !!! ***
;
...
;;hetg_rs_flight = 8632.65 ; * 1.000470 ; mm  7/13 470 ppm to absolute?
;;letg_rs_flight = 8632.31 ; mm
; 8/16/00: Change RS to 8637 mm for better accuracy while 
; pixel size, etc. calibration is on-going.
hetg_rs_flight = 8637.00 ; mm
letg_rs_flight = 8637.00 ; mm
; at XRCF:
...
if KEYWORD_SET(ZBUFFER) then begin
  if KEYWORD_SET(MOUSE) then begin
    print, ''
    print, ' *** MOUSE option not available with ZBUFFER! ***'
    print, '     Setting MOUSE to 0 '
    print, ''
    MOUSE=0
  end
  ...
  if KEYWORD_SET(OVERRIDE_ZERO) then begin
    print, ''
    print, ' OVERRIDE_ZERO set with ZBUFFER: will pop up window when click is needed.'
    print, ''
  end
end
...
; Option to re-analyze using the previously EXPORTed to common
; events structure
if KEYWORD_SET(REANAL) then begin
  if NOT(KEYWORD_SET(SILENT)) then begin
    print, ' Source of data: EXPORTed evts'
    print, ''
  end
  printf,out_unit, ' Source of data: EXPORTed evts'
  printf,out_unit, ''
  ; read back the events...
  evts = oa_evts
  filename = oa_filename
...
; - - end of file input and REANAL option - -
end
...
; BS 9/18/00 added link to obsid info
;  assumes naming scheme 'obsid_xxxx'
output_obsid = strarr(2)
output_obsid = strsplit(output_prefix,'_', /extract)
printf, out_unit, '  <H2 align="center">'+output_obsid(0)+'_<a href="http://acis.mit.edu/cgi-bin/get-obsid?id=0'+output_obsid(1)+'">'+output_obsid(1)+'</a>_'+proc_method+'</H2>'
...
; IF not skipped!
if NOT(KEYWORD_SET(SKIPSEL)) then begin
; - - -
...
; If ZBUFFER is used AND OVERRIDE_ZERO is selected
; then the following plots will have to be done to X window
; so that operator can interact with them...
if KEYWORD_SET(ZBUFFER) AND KEYWORD_SET(OVERRIDE_ZERO) then begin
  set_plot, 'X'
  window, iwind, xsi=700, ysi=700
  dd_load_ct, RED=red_ct, GREEN=green_ct, BLUE=blue_ct, /OTHER
end
...
; Plot the events ...
; Compute the Average Xtouse and Ytouse, but do it before bothering
;  with a plot to click
nevts = n_elements(evts(sel))
aveXtouse = TOTAL(Xtouse(sel))/FLOAT(nevts)
aveYtouse = TOTAL(Ytouse(sel))/FLOAT(nevts)

maxplotpnts = 20000
if (n_elements(sel) gt maxplotpnts) then selplot = sel( lindgen(maxplotpnts)*(n_elements(sel)/maxplotpnts) )
if (n_elements(sel) le maxplotpnts) then selplot = sel
plot, Xtouse(selplot), Ytouse(selplot), PSYM=3, $
	XSTYLE=1, YSTYLE=1, $
	BACK=clr_back, COLOR=clr_wht, /NODATA, $
        TITLE='X-Y Plot of Selected Events', $
        XTITLE=x_name+' (pixels)', $
        YTITLE=y_name+' (pixels)', $
	CHARSIZE=1.11
plots, Xtouse(selplot), Ytouse(selplot), PSYM=3, COLOR=energy_colors(selplot), NOCLIP=0
...
plot, Xtouse(selplot), Ytouse(selplot), PSYM=3, $
	...
	BACK=clr_back, COLOR=clr_wht, /NODATA, $
        ...
        YTITLE=y_name+' (pixels)', $
	CHARSIZE=1.11
plots, Xtouse(selplot), Ytouse(selplot), PSYM=3, COLOR=energy_colors(selplot), $
	NOCLIP=0
...
; Finish the .gif output
; Put window upfront if on screen
if NOT(KEYWORD_SET(ZBUFFER)) then wshow,iwind,iconic=0
; If ZBUFFER was used with OVERRIDE_ZERO then
; this plot was done to screen and need to put it in front too
if KEYWORD_SET(ZBUFFER) AND KEYWORD_SET(OVERRIDE_ZERO) then begin
  wshow,iwind,iconic=0
end
image = tvrd()
...
; Now need to go back to ZBUFFER if that is desired
if KEYWORD_SET(ZBUFFER) AND KEYWORD_SET(OVERRIDE_ZERO) then begin
  set_plot, 'Z' 
  device, set_resolution=[700,700]
  dd_load_ct, RED=red_ct, GREEN=green_ct, BLUE=blue_ct, /OTHER
...
for iq=iqstart,iqstop do begin
  ...
  CASE iq OF
    0: begin
		...
		(AY GT (aveAY+(cd_offset/pixel_size) - $
			0.5*cd_width/pixel_size)) AND $
    ...
    2: begin
         if cc_mode then begin
           ...
           sel = where(AX LT aveAX - zoregsize/2.0 AND $
		(AY LT (aveAY+(cd_offset/pixel_size) + $
			0.5*cd_width/pixel_size)) AND $
		ABS(AY - Yaxis ) LT (0.5*cd_width/pixel_size) )
         end
         megmsel = sel
...
if NOT(KEYWORD_SET(SILENT)) then begin
  ...
  if GRATING EQ 'LETG' then begin
    print, ' LETG period is '+ $
	STRING(p_leg,FORMAT='(F8.2)')+' A'
  end
  if GRATING EQ 'HETG' then begin
    print, ' MEG period is '+ $
	STRING(p_meg,FORMAT='(F8.2)')+' A'
    print, ' HEG period is '+ $
	STRING(p_heg,FORMAT='(F8.2)')+' A'
  end
  print, ''
end
...
printf, out_unit, ' Rowland spacing used is '+ $
	STRING(hetg_rs,FORMAT='(F8.2)')+' mm'
  if GRATING EQ 'LETG' then begin
    printf, out_unit, ' LETG period is '+ $
	STRING(p_leg,FORMAT='(F8.2)')+' A'
  end
  if GRATING EQ 'HETG' then begin
    printf, out_unit, ' MEG period is '+ $
	STRING(p_meg,FORMAT='(F8.2)')+' A'
    printf, out_unit, ' HEG period is '+ $
	STRING(p_heg,FORMAT='(F8.2)')+' A'
  end
printf, out_unit, ''
...
if KEYWORD_SET(EXPORT) then begin
  ...
  oa_output_prefix = output_prefix
  oa_evts = evts
  oa_ftime = ftime
  oa_Xtouse = Xtouse
  oa_Ytouse = Ytouse
