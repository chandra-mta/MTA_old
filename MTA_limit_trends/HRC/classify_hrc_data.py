...
def separate_data(infile, cut):
    ...
    for ent in data:
        ...
        stime2 = int(float(atemp[1]))
        if (stime >= cut) or (stime < cut and stime2 > cut):
            start.append(stime)
...
#
#--- just in a case the file disappeared
#
    check_file_cotent(tsfile)
    check_file_cotent(spfile)
    check_file_cotent(imfile)
#
#--- create backup
#
    cmd    = 'cp ' +  tsfile + ' ' + tsfile + '~'
    os.system(cmd)
    cmd    = 'cp ' +  spfile + ' ' + spfile + '~'
    os.system(cmd)
    cmd    = 'cp ' +  imfile + ' ' + imfile + '~'
    os.system(cmd)
#
...
#
    try:
        ...
        for infile in (tsfile, spfile, imfile):
            data   = mcf.read_data_file(tsfile)
...
#
    ...
    for k in range(0, len(off_start)):
        ...
        line = line + str(int(off_start[k])) + ':' + str(int(off_stop[k])) + ':' + str(diff) +  '\n'
